...
class AdvancedBot(BaseBot):
    ...
    def load_config(self):
        ...
        if loaded is not None:
            ...
            self.config["welcome_messages"] = [self.config["welcome_message"]]
            # 🌐 اگه از سایت زبون انگلیسی برای این بات انتخاب شده (ساخت اولیه)، پیام خوش‌آمد
            # پیش‌فرض هم انگلیسی باشه، نه فارسی/میکس.
            if os.getenv("BOT_LANGUAGE", "").strip().lower() == "en":
        ...
        if env_lang in ("fa", "en") and self.config.get("language") != env_lang:
    ...
    async def cmd_lang(self, user: User, parts: list):
        ...
        if len(parts) < 2 or parts[1].lower() not in ("fa", "en"):
            ...
            await self.chat("⚠️ فرمت: !lang fa یا !lang en" if lang_now == "fa" else "⚠️ Usage: !lang fa | !lang en")
            return
    ...
    async def cmd_language(self, user: User, parts: list):
        """!language fa / !language en -> تغییرِ زبانِ بات.
        ...
        if user.username.lower() not in self.config["admin_usernames"]:
            ...
            return
        if len(parts) != 2 or parts[1].lower() not in ("fa", "en"):
            await self.chat("⚠️ استفاده کن از: !language fa یا !language en" if self.config.get("language", "fa") == "fa"
                             else "⚠️ Use: !language fa or !language en")
            return
        self.config["language"] = parts[1].lower()
        self.save_config()
        await self.chat(self.get_message("lang_changed") if self.config["language"] == "fa"
    ...
    async def cmd_addhost(self, user: User, parts: list):
        """⚠️ اختصاصی: فقط خود مالک اصلی بات (ahoora_king) می‌تواند رتبه Host بدهد؛
        حتی سایر Host‌ها هم اجازه اجرای این دستور را ندارند."""
        lang = self.config.get("language", "fa")
        if user.username.lower() != "ahoora_king":
            await self.chat("❌ دسترسی غیرمجاز!" if lang == "fa" else "❌ Unauthorized!")
    ...
    async def cmd_removehost(self, user: User, parts: list):
        """⚠️ اختصاصی: فقط خود مالک اصلی بات (ahoora_king) می‌تواند رتبه Host را بگیرد."""
        lang = self.config.get("language", "fa")
        if user.username.lower() != "ahoora_king":
            await self.chat("❌ دسترسی غیرمجاز!" if lang == "fa" else "❌ Unauthorized!")
    ...
    async def cmd_listadd(self, user: User, parts: list):
        ...
        try:
            ...
            await self.chat((f"خطا در نمایش لیست ادمین\u200cها: {str(e)}" if self.config.get("language","fa") == "fa" else f"Error showing admin list: {str(e)}"))
